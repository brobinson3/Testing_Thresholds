...
dfi = pd.read_csv('Sites_Master_List_Final.csv', delimiter=',', header=None, skiprows=1)
sites = dfi[0].values
time = 50
cutoff = 0.6
agreement = 0.6
...
danger_year = pd.DataFrame(0, columns=sites, index=range(1,11))
site_count = 0
...
for site in sites:
  ...
  lowest_all_data = cmip_annQ_spag[lowest_all.index]
...
# FILTER
  ...
  half_half_end = half_half_end.interpolate(axis=1)
  half_half_end.iloc[0,:] = half_half_end.iloc[0,:].fillna(value=0)
  for year in range(0,CMIP5_filtered.shape[0]):
    if half_half_end.iloc[0,year] == 0:
        drought_error_count += 1

  for label in range(0,lowest_all.shape[0]):
    for year in range(0,CMIP5_filtered.shape[0]):
      if half_half_end.iloc[0,year] != 0:
        if lowest_all_data.iloc[year,label] < half_half_end.iloc[0,year]:
          danger_year.iloc[label,site_count] = year+1999
          break
      
  print(site_count+1,site,danger_year[site])
  site_count += 1

print(danger_year)
danger_year.to_csv('More_Rivers2_{}-yr_Danger_Cross_Dates_with_{}-lowest_and_{}%_agreement_with-interpolation_all.csv'.format(time,danger_count,agreement*100))
